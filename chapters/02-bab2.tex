...
\paragraph{Perbandingan dengan GNN Tradisional}

Untuk membandingkan dengan GNN tradisional yang menggunakan rata-rata sederhana (uniform weights):

\begin{align}
    \vec{h'}_A^{\text{GNN}} &= \text{ReLU}\left(\frac{1}{3}\vec{z}_A + \frac{1}{3}\vec{z}_B + \frac{1}{3}\vec{z}_C\right) \\
    &= \text{ReLU}\left(\frac{1}{3}\begin{bmatrix} 0.55 \\ 0.65 \end{bmatrix} + \frac{1}{3}\begin{bmatrix} 0.90 \\ 0.75 \end{bmatrix} + \frac{1}{3}\begin{bmatrix} 0.60 \\ 0.45 \end{bmatrix}\right) \\
    &= \text{ReLU}\left(\begin{bmatrix} 0.683 \\ 0.617 \end{bmatrix}\right) = \begin{bmatrix} 0.683 \\ 0.617 \end{bmatrix}
\end{align}

\noindent
\textbf{Analisis perbedaan}:
\begin{itemize}
    \item GAT: $\vec{h'}_A = [0.694, 0.625]^T$
    \item GNN tradisional: $\vec{h'}_A^{\text{GNN}} = [0.683, 0.617]^T$
    \item Selisih: $[0.011, 0.008]^T$
\end{itemize}

Meskipun perbedaannya terlihat kecil dalam contoh sederhana ini, dalam graf kompleks dengan ratusan node dan koneksi yang heterogen (seperti jaringan PT Lare Osing Ndo), mekanisme atensi GAT memberikan keunggulan signifikan dengan:
\begin{enumerate}
    \item Memberikan bobot lebih tinggi pada jalur berkualitas tinggi
    \item Secara adaptif mengabaikan jalur yang mengalami degradasi
    \item Menangkap pola yang lebih kompleks dalam topologi jaringan
\end{enumerate}
