...
\begin{enumerate}
    ...
    \item \textbf{Measurement:} Keberhasilan metode diukur menggunakan metrik statistik \textit{Root Mean Square Error} (RMSE) untuk performa regresi, dan \textit{Accuracy} dengan toleransi error ($\pm 0.05, \pm 0.08, \pm 0.10$) untuk performa klasifikasi kualitas.
\end{enumerate}

% (Kode gambar TikZ Anda yang sebelumnya sudah bagus, tidak perlu diubah kecuali teks di dalam kotak Indicators jika Anda ingin sangat detail, tapi deskripsi teks di atas sudah cukup mewakili).

Visualisasi kerangka pemikiran penelitian ditunjukkan pada Gambar \ref{fig:kerangka_pemikiran}.

\begin{figure}[ht]
    \centering
    % Resizebox digunakan agar gambar otomatis mengikuti lebar teks (textwidth)
    \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        node distance=0.5cm,
    ...
    \node[box, below=0.5cm of met_title, fill=gray!10, minimum width=4cm] (data) {Dataset Generation\\(Topologi \& Metrik)};
    \node[box, below=0.8cm of data, fill=gray!10, minimum width=4cm] (ahp) {\textbf{Weighting Strategy}\\Analytic Hierarchy Process\\(AHP)};
    \node[box, below=0.8cm of ahp, fill=gray!20, minimum width=4cm, minimum height=1.5cm] (gat) {\textbf{Learning Algorithm}\\Graph Attention Network\\(3 Layers, 4 Heads)};
    ...
    \node[box, below=0.5cm of rmse] (acc) {Accuracy\\($\pm 0.05, \pm 0.08$)};
